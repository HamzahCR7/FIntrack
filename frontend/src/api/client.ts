...
export const api = {
  ...
  getQuickItems: async (): Promise<QuickItem[]> => {
    const res = await axios.get(`${API_BASE}/quick-items`);
    return res.data.data;
  },
  ...
  createQuickItem: async (data: { type: QuickItemType; title: string; details?: string; price?: number; category?: string; priority?: QuickItemPriority; dueDate?: string }): Promise<QuickItem> => {
    const res = await axios.post(`${API_BASE}/quick-items`, data);
    return res.data.data;
  },

  updateQuickItem: async (id: string, data: Partial<QuickItem>): Promise<QuickItem> => {
    const res = await axios.patch(`${API_BASE}/quick-items/${id}`, data);
    return res.data.data;
  },

  deleteQuickItem: async (id: string): Promise<void> => {
    await axios.delete(`${API_BASE}/quick-items/${id}`);
  },
  ...
  getForecast: async () => {
    const res = await axios.get(`${API_BASE}/analytics/forecast`);
    return res.data.data;
  },
  ...
  getRunwaySimulation: async (payload: {
    ...
    const res = await axios.post(`${API_BASE}/analytics/runway-simulation`, payload);
    return res.data.data;
  },
  ...
  updateFinancialProfile: async (data: Record<string, number>) => {
    const res = await axios.patch(`${API_BASE}/profile`, data);
    return res.data.data;
  },

  saveInsightFeedback: async (insightId: string, isUseful: boolean) => {
    await axios.post(`${API_BASE}/analytics/insights/${insightId}/feedback`, { isUseful });
  },
  ...
  getCreditCards: async (): Promise<Account[]> => {
    const res = await axios.get(`${API_BASE}/credit-cards`);
    return res.data.data;
  },
  ...
  getUpcomingSubscriptions: async (): Promise<Subscription[]> => {
    const res = await axios.get(`${API_BASE}/subscriptions/upcoming`);
    return res.data.data;
  },
  ...
  createDebt: async (data: Partial<Debt> & { accountId?: string }): Promise<Debt> => {
    const res = await axios.post(`${API_BASE}/debts`, data);
    return res.data.data;
  },

  updateDebt: async (id: string, data: Partial<Debt>): Promise<Debt> => {
    const res = await axios.patch(`${API_BASE}/debts/${id}`, data);
    return res.data.data;
  },

  settleDebt: async (id: string, amountToSettle: number, accountId?: string): Promise<Debt> => {
    const res = await axios.post(`${API_BASE}/debts/${id}/settle`, { amountToSettle, accountId });
    return res.data.data;
  },
  ...
  getTransactions: async (filters?: TransactionFilters): Promise<Transaction[]> => {
    const res = await axios.get(`${API_BASE}/transactions`, { params: filters });
    return res.data.data;
  },
  ...
  parseReceipt: async (data: { imageBase64: string; mimeType?: string }): Promise<ParsedReceipt> => {
    const res = await axios.post(`${API_BASE}/receipts/parse`, data);
    return res.data.data;
  },
  ...
  getBudgetById: async (id: string) => {
    const res = await axios.get(`${API_BASE}/budgets/${id}`);
    return res.data.data;
  },

  getBudgetStatus: async (id: string) => {
    const res = await axios.get(`${API_BASE}/budgets/status/${id}`);
    return res.data.data;
  },
  ...
  updateBudget: async (id: string, data: any) => {
    const res = await axios.patch(`${API_BASE}/budgets/${id}`, data);
    return res.data.data;
  },
  ...
  getActiveGoals: async () => {
    const res = await axios.get(`${API_BASE}/goals/active`);
    return res.data.data;
  },

  getGoalById: async (id: string) => {
    const res = await axios.get(`${API_BASE}/goals/${id}`);
    return res.data.data;
  },

  createGoal: async (data: any) => {
    const res = await axios.post(`${API_BASE}/goals`, data);
    return res.data.data;
  },

  updateGoal: async (id: string, data: any) => {
    const res = await axios.patch(`${API_BASE}/goals/${id}`, data);
    return res.data.data;
  },

  updateGoalProgress: async (id: string, currentAmount: number) => {
    const res = await axios.post(`${API_BASE}/goals/${id}/progress`, { currentAmount });
    return res.data.data;
  },

  incrementGoalProgress: async (id: string, amount: number) => {
    const res = await axios.post(`${API_BASE}/goals/${id}/increment`, { amount });
    return res.data.data;
  },

  completeGoal: async (id: string) => {
    const res = await axios.post(`${API_BASE}/goals/${id}/complete`);
    return res.data.data;
  },

  pauseGoal: async (id: string) => {
    const res = await axios.post(`${API_BASE}/goals/${id}/pause`);
    return res.data.data;
  },

  resumeGoal: async (id: string) => {
    const res = await axios.post(`${API_BASE}/goals/${id}/resume`);
    return res.data.data;
  },
